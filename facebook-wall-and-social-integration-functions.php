...
if($msfb_max==0){			
	...
	$msfb_html_content_first .= '<span class="msfb-wall-message">Call to Facebook API failed or no records returned. Make sure system requirements are met(look for the tab in settings page), facebook id and access token(<a target="_blank" href="https://developers.facebook.com/tools/debug/">check here</a>) are correct. Finally, make sure if there are some restrictions set in facebook page/group/profile settings comparing with other pages/groups for which feed display works.</span>';
